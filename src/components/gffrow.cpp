...
void GffRow::parse(std::string& gffrow)
{
  ...
  if(strand < 0) { std::cerr << "[ Warning ]: Bad value for strand\n"; }
  ...
  else { id = std::to_string(anon_feat_count++); }
...
void GffRow::parse_attributes(std::string& attribute_line)
{
  ...
  for(std::string& i : stringtools::tokenize(attribute_line, ';'))
  {
    ...
    std::vector<std::string> comment
      = stringtools::tokenize(stringtools::strip(i), '=');
    if(comment.size() != 2)
    {
    ...
    else
    {
      const std::vector<std::string> values
        = stringtools::tokenize(comment[1], ',');
